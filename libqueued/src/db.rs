...
fn rocksdb_opts(data_dir: &Path) -> fjall::Config {
  // https://github.com/facebook/rocksdb/wiki/Setup-Options-and-Basic-Tuning#other-general-options.
  let mut opt = fjall::Config::new(data_dir);
  ...
  opt = opt.max_write_buffer_size(1024 * 1024 * 1024 * 1);
  ...
  opt = opt.manual_journal_persist(true);

  // opt.set_compression_type(rocksdb::DBCompressionType::None);
  // NOTE: in fjall, this is per partition
  ...
  opt = opt.block_cache(Arc::new(BlockCache::with_capacity_bytes(
    1024 * 1024 * 1024 * 1,
  )));
...
pub(crate) fn rocksdb_open(data_dir: &Path) -> Keyspace {
  rocksdb_opts(data_dir).open().unwrap()
}
